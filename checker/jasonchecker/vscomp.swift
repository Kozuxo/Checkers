...
struct home: View {
        ...
        ScrollView {
            VStack{
               ...
               ZStack{
                ...
                HStack{
                    Button(action: {komputermode = false}, label: {
    ...
    func warnaterkam(){
        for he in heal{
            if bulet[he.x][he.y].transparan == 0.3{
                ...
                bulet[he.x][he.y].color = Color.gray
            }
        }
        heal = [coordinate]()
    }
    ...
    func highlight(){
        for mu in musuh{
            ...
            bulet[mu.x][mu.y].color = Color.gray
        }
        musuh = [coordinate]()
    }
    ...
    func checkselect(i:Int, j:Int){
        ...
        if makan.count != 0{
            ...
            if bulet[i][j].dipilih == true && bulet[i][j].color == warnagiliran && mencari == false{
                ...
                if warnagiliran == Color.orange{
                 let ind = Int.random(in: 0...musuh.count - 1)
                    checkselect(i: musuh[ind].x, j: musuh[ind].y)
                }
            }
                ...
                bulet[tukartempat.x][tukartempat.y].color = Color.gray
                ...
                bulet[(tukartempat.x+i)/2][(tukartempat.y + j)/2].color = Color.gray
                warnamenang()
                ...
                if musuh.count == 0{
                    ...
                    if enemycolor == Color.orange{
                        if makan.count == 0{
                            for col in 0...7{
                                for row in 0...7{
                                    if black_finished == false && bulet[col][row].color == Color.orange{
                            ...
                            black_finished = false
                         let tmp = Int.random(in: 0...makan.count-1)
                            checkselect(i: makan[tmp].x, j: makan[tmp].y)
                        }
                    }
                }
                else{
                    ...
                    if warnagiliran == Color.orange{
                        mencari = false
                        black_finished = false
                        checkselect(i: makan[0].x, j: makan[0].y)
                    }
                }
            }
        }
...
struct PveView_Previews: PreviewProvider {
    static var previews: some View {
        home(komputermode: .constant(true), sekarang: .constant(.pve))
    }
}
